...
window.addEventListener("load", () => {
   ...
   // startWorker(original_img_data, "gaussian");
...
function startWorker(original_img_data, type) {
    if(typeof(Worker) !== "undefined") 
    {
        if(typeof(w) == "undefined") 
        {
            myWorker = new Worker("pre_compute.js");
            if(type == "gaussian")
                myWorker.postMessage([original_img_data, canvas_width, max_gaussian, type]);
            else if(type == "modulus") myWorker.postMessage([original_img_data, canvas_width, max_modulus, type]);
        }
        if(type == "gaussian")
        {
            myWorker.addEventListener("message", function handleMessageFromWorker(msg) {
                ultra_computed_gaussian_blur = msg.data;
                if(ultra_computed_gaussian_blur.length >= max_gaussian)
                {
                    stopWorker();
                    startWorker(original_img_data, "modulus");
                }
        ...
        {
            myWorker.addEventListener("message", function handleMessageFromWorker(msg) {
                ultra_computed_modulus = msg.data;
                if(ultra_computed_modulus.length >= max_modulus)
                    stopWorker();
            });
        }
