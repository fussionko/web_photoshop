...
function draw_graph_animation()
{
    ...
    for(let range in graph["r"])
    {
        ...
        if(i <= graph["r"][range]) 
        {
            ...
            // graph_ctx.stroke();
            // graph_ctx.closePath();
            console.log(graph_offset+range+1, graph_offset+range+2, graph_offset+range+3);
            graph_ctx.beginPath();
            graph_ctx.strokeStyle = "#FF0000";
            graph_ctx.moveTo(graph_offset+range, graph_height);
            graph_ctx.bezierCurveTo(graph_offset+range+1, graph_height, graph_offset+range+2, graph_height-i, graph_offset+range+3, graph_height-i);
