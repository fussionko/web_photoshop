...
function draw_graph()
{
    ...
    for(let range in graph["r"])
    {
        ...
        console.log(graph["r"][range]);
        console.log(5+i, y, range+i+5, y-graph["r"][range]);
        graph_ctx.beginPath();
        graph_ctx.moveTo(5+i, y);
        graph_ctx.lineTo(range+5+i, y-graph["r"][range]);
        graph_ctx.stroke();
        i++
    }
