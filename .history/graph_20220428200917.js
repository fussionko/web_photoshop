...
function create_graph()
{
    ...
    console.log("create _ graph");
    
}

async function count_Pixel(img_data)
{
    console.log(img_data, "notr");
...
function draw_graph_animation()
{
    ...
    for(let range in graph["r"])
    {
        ...
        drawCurve(range, graph_height-graph["r"][range], prev_r, "#FF0000")
        drawCurve(range, graph_height-graph["g"][range], prev_g, "#00FF00")
        drawCurve(range, graph_height-graph["b"][range], prev_b, "#0000FF")
        prev = graph["r"][range];
    }
    graph_ctx.beginPath();
    graph_ctx.moveTo(graph_offset+255, graph_height-prev_r);
    graph_ctx.quadraticCurveTo(graph_offset+255, graph_height-prev_r, graph_offset+256, graph_height);
    graph_ctx.stroke();
    graph_ctx.closePath();

    graph_ctx.beginPath();
    graph_ctx.moveTo(graph_offset+255, graph_height-prev_g);
    graph_ctx.quadraticCurveTo(graph_offset+255, graph_height-prev_g, graph_offset+256, graph_height);
    graph_ctx.stroke();
    graph_ctx.closePath();

    graph_ctx.beginPath();
    graph_ctx.moveTo(graph_offset+255, graph_height-prev_b);
    graph_ctx.quadraticCurveTo(graph_offset+255, graph_height-prev_b, graph_offset+256, graph_height);
    graph_ctx.stroke();
    graph_ctx.closePath();
